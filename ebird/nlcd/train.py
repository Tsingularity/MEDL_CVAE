...
def main(_):
	...
	learning_rate = tf.train.exponential_decay(FLAGS.learning_rate,global_step,0.25*FLAGS.max_epoch*len(train_order)/FLAGS.batch_size,0.5,staircase=True)
	...
	def train_step(input_nlcd,input_label,smooth_ce_loss,smooth_l2_loss,smooth_total_loss):
		...
		if step%100==0:
			ce_loss=smooth_ce_loss/100.0
			l2_loss=smooth_l2_loss/100.0
			total_loss=smooth_total_loss/100.0
			tempstr = "{}: step {}, ce_loss {:g}, l2_loss {:g}, total_loss {:g}".format(time_str, step, ce_loss, l2_loss, total_loss)
			print(tempstr)
			return output,0.0,0.0,0.0

		return output,smooth_ce_loss,smooth_l2_loss,smooth_total_loss

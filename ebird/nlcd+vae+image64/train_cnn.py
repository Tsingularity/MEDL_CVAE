...
tf.app.flags.DEFINE_string('model_dir','./model_cnn/','path to store model')
tf.app.flags.DEFINE_string('summary_dir','./summary_cnn','path to store summary_dir')
...
def main(_):
	...
	hg = vae_cnn.vae(is_training=True)
	...
	for one_epoch in range(FLAGS.max_epoch):
		...
		for i in range(int(len(train_order)/float(FLAGS.batch_size))):
			...
			input_image = get_data.get_jpg_test(jpg_list,train_order[start:end])/128.0
